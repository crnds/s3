/*
 * Vendored from the seller's JC3248W535EN demo package
 * (1-Demo/Demo_Arduino/DEMO_LVGL/esp_lcd_axs15231b.c, Espressif, Apache-2.0),
 * trimmed to the PANEL half only -- touch is read directly over I2C in
 * src/touch_axs.cpp, so the esp_lcd_touch dependency is gone. The default init
 * table is replaced with the one the seller's esp_bsp.c actually sends
 * (lcd_init_cmds[]), which is what every shipped demo runs.
 *
 * QSPI quirk worth knowing (see draw_bitmap below): in QSPI mode the driver
 * sends only CASET, never RASET. A window starting at y=0 is written with
 * RAMWR (0x2C, restarts at row 0); any other y uses RAMWRC (0x3C, continues
 * from wherever the previous write stopped). So the panel is effectively a
 * top-to-bottom stream: partial updates are NOT addressable -- always push
 * whole frames in order, starting at row 0.
 */
